...
from ffreed.env.reward import Reward, MolLogP, PatternFilter, OutOfRange, identity, MolSAS, fraction, docking
...
def evaluate(args, epoch, env):
    ...
    mols = read_mols(args, epoch)
    rewards = env.reward_batch(mols, weight)
    suffix = int2str(epoch)
...
def init_rewards(args):
    ...
    # Wrap properties with cache
    cached = lambda f, p=1: CacheAndPool(f, processes=p)

    weight = args['weights']

    costs = {
        'DockingScore': Reward(cached(DockingVina(args['docking_config'])), docking, weight=weight),
        'SAS': Reward(cached(MolSAS), fraction, weight=(1.0 - weight), preprocess=preprocess),
    }

    rewards = {k: v for k, v in costs.items() if k in args['objectives']}

    # costs = {
    #     'DockingScore': Reward(DockingVina(args['docking_config']), docking),
    #     'SAS': Reward(MolSAS, fraction, preprocess=preprocess),
    #     'LogP': Reward(MolLogP, OutOfRange(lower=0, upper=5, hard=hard), args['weights'], preprocess=preprocess),
    #     'HeavyAtomCount': Reward(Lipinski.HeavyAtomCount, OutOfRange(upper=40, hard=hard), args['weights'], preprocess=preprocess),
    #     'NumHAcceptors': Reward(Lipinski.NumHAcceptors, OutOfRange(upper=10, hard=hard), args['weights'], preprocess=preprocess),
    #     'NumHDonors': Reward(Lipinski.NumHDonors, OutOfRange(upper=5, hard=hard), args['weights'], preprocess=preprocess),
    #     'PAINS': Reward(PatternFilter(patterns['PAINS']), identity, args['weights'], preprocess=preprocess),
    #     'SureChEMBL': Reward(PatternFilter(patterns['SureChEMBL']), identity, args['weights'], preprocess=preprocess),
    #     'Glaxo': Reward(PatternFilter(patterns['Glaxo']), identity, args['weights'], preprocess=preprocess),
    # }
    # rewards = dict()

    # for name in args['objectives']:
    #     if costs[name] == 'DockingScore':
    #         costs[name].weight = args['weights']
    #     else:
    #         costs[name].weight = 1.0 - args['weights']
...
def init_models(args, env, checkpoint=None):
    ...
    log_w1 = torch.tensor([np.log(args['weights'])], requires_grad=True, device=device)
    ...
    if checkpoint:
        ...
        log_w1 = torch.tensor([checkpoint['log_w1']], requires_grad=args['train_alpha'], device=device)

        if prioritizer:
            prioritizer.load_state_dict(checkpoint['prioritizer'])
    
    return actor, critic, critic_target, log_alpha, log_w1, prioritizer


def init_optimizers(actor, critic, critic_target, log_alpha, log_w1, prioritizer, args, checkpoint=None):
    ...
    #args['weights'] = torch.tensor(args['weights'], requires_grad=True)
    weight_optimizer = Adam([log_w1], lr=0.01)
    ...
    return actor_optimizer, critic_optimizer, alpha_optimizer, prioritizer_optimizer, weight_optimizer
