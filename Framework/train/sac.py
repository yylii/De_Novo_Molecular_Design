...
class SAC:
    ...
    def __init__(self, actor, critic, critic_target, log_alpha, log_w1, prioritizer, 
                actor_optimizer, critic_optimizer, alpha_optimizer, prioritizer_optimizer, 
                weight_optimizer, env, replay_buffer, writer, 
        ...
        # Initialize w1 as a learnable parameter
    ...
    def critic_loss(self, data):
        ...
        return {
            #'critic_loss': (w1 * loss_critic).mean()
            'critic_loss': (loss_critic).mean()
        }
    ...
    def actor_loss(self, data):
        ...
        return {
            'actor_loss': (loss_actor).mean(),
            'entropy_loss': (loss_entropy).mean(),
            'policy_loss': (loss_policy).mean(),
            'alpha_loss': (loss_alpha).mean(),
    ...
    def update_weight(self, data):
        ...
        reward = reward.detach().requires_grad_(True)
        loss = -reward.mean()
        loss.backward()
    ...
    def collect_experience(self):
        ...
        print(f"rewards['Reward']={rewards['Reward']}")
    ...
    def compute_rewards(self, smiles):
        w1 = self.log_w1.exp().item()
        print(f"w1 in SAC: {w1}")
        return self.env.reward_batch(smiles, w1=w1)
    ...
    def save_model(self):
        ...
        print(f'w1 in save_model: {self.log_w1.item()}')
